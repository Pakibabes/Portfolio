...
export default function ProjectCard({
  ...
  return (
    <Reveal delay={delay}>
      <article className="group relative flex h-full flex-col gap-4 rounded-2xl border border-panel-line bg-panel p-6 transition-colors hover:border-violet/50 md:p-8">
          ...
          {project.tags.map((tag) =>
            tag.href ? (
              <a
                ...
                href={tag.href}
                target="_blank"
                className="rounded-full border border-panel-line px-3 py-1 font-mono text-xs text-muted transition-colors hover:border-cyan hover:text-cyan"
              >
                {tag.label}
              </a>
            ) : (
              <span
                key={tag.label}
                className="rounded-full border border-panel-line px-3 py-1 font-mono text-xs text-muted"
              >
                {tag.label}
              </span>
            )
